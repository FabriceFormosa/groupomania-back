...
async function deleteUser(req, res) {
  ...
  console.log("----------------------------- start fonction deleteUser ------------------------------")
  try {
    ...
    for await (const post of posts) {
      ...
      console.log("deletePost nbCommentDeleted : ",nbCommentDeleted," postId : ",post.id)

      /// delete the post
      const onePost = await prisma.Posts.delete({where: { id: post.id }});
      console.log("----- deletePost onePost :",onePost," postId : ",post.id)
    }

    // rechercher les commentaires liés au user
    const comments = await prisma.Comments.deleteMany({where: { userId: user_Id }});
    console.log("------------ userDeleted comments :",comments)
  ...
  console.log("----------------------------- end fonction deleteUser ------------------------------")
}
